...
ssr_link_qr(){
	...
	Remarksbase64=$(urlsafe_base64 "Share")
	SSRbase64=$(urlsafe_base64 "${ip}:${port}:${SSRprotocol}:${method}:${SSRobfs}:${SSRPWDbase64}/?obfsparam=&protoparam=&remarks=${Remarksbase64}&group=SEs")
	SSRurl="ssr://${SSRbase64}"
	Suburl=$(urlsafe_base64 "${SSRurl}")
	SSRQRcode="http://doub.pw/qr/qr.php?text=${SSRurl}"
	ssr_link=" SSR   链接 : ${Red_font_prefix}${SSRurl}${Font_color_suffix} \n SSR订阅链接 : ${Red_font_prefix}${Suburl}${Font_color_suffix} \n SSR 二维码 : ${Red_font_prefix}${SSRQRcode}${Font_color_suffix} \n "
}
ss_ssr_determine(){
...
Set_config_user(){
	...
	stty erase '^H' && read -p "(默认: Share):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="Share"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	stty erase '^H' && read -p "(默认: proud):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="proud"
...
if [[ "${action}" == "clearall" ]]; then
  ...
  ---- Toyo | doub.io/ss-jc60 | Yefori 修改 ----
